  ...
  return (
    <View style={{width: s, height: s}}>
      <View
        style={[
          styles.avatarCircle,
          {width: s, height: s, borderRadius: s / 2, backgroundColor: bg + '18'},
        ]}>
        <Text style={{fontSize, color: bg, fontWeight: '600'}}>
          {icon || initials || '?'}
        </Text>
      </View>
      {status && (
  ...
  return (
    <DemoCard
      ...
      <View style={styles.row}>
        ...
        {/* Label badges */}
        <View style={[styles.labelBadge, {backgroundColor: Colors.success + '18'}]}>
          <Text style={[styles.labelBadgeText, {color: Colors.success}]}>Active</Text>
        </View>
        <View style={[styles.labelBadge, {backgroundColor: Colors.warning + '18'}]}>
          <Text style={[styles.labelBadgeText, {color: Colors.warning}]}>Pending</Text>
        </View>
        <View style={[styles.labelBadge, {backgroundColor: Colors.error + '18'}]}>
    ...
    <View style={styles.variationCard}>
      ...
      <Pressable onPress={() => setExpanded(!expanded)}>
        <View style={styles.variationRow}>
          <View style={{flex: 1}}>
            ...
            <Text style={styles.variationSub}>Click to {expanded ? 'collapse' : 'expand'}</Text>
          </View>
      ...
      <Animated.View style={{height: bodyHeight, opacity: bodyOpacity, overflow: 'hidden'}}>
        <Text style={styles.variationBody}>
          This card reveals additional content when clicked. Useful for FAQs, settings groups, or detail sections that should stay out of the way.
        </Text>
      </Animated.View>
    </View>
  );
    ...
    <DemoCard
      ...
      <View style={styles.variationCard}>
        ...
        <View style={styles.actionRow}>
          <Pressable style={[styles.actionBtn, {backgroundColor: Colors.primary + '12'}]}>
            <Text style={[styles.actionBtnText, {color: Colors.primary}]}>Share</Text>
          </Pressable>
          <Pressable style={[styles.actionBtn, {backgroundColor: Colors.accent + '12'}]}>
            <Text style={[styles.actionBtnText, {color: Colors.accent}]}>Save</Text>
          </Pressable>
          <Pressable style={[styles.actionBtn, {backgroundColor: Colors.error + '12'}]}>
...
function ListItemDemo() {
  ...
  return (
    <DemoCard
      eyebrow="LIST ITEM"
      title="List Items"
      description="Rich list rows with icon, subtitle, trailing action, and hover state.">
      <View style={styles.listContainer}>
        {LIST_ITEMS.map((item, idx) => (
          <Pressable
            ...
            style={[
              styles.listItem,
              hoveredIdx === idx && styles.listItemHovered,
            ]}>
...
const ACCORDION_ITEMS = [
  {title: 'What technologies are used?', body: 'React Native Windows 0.75 with TypeScript, React Navigation 7, Fluent Design, and the Animated API for all motion.'},
          ...
          <View key={i} style={styles.timelineRow}>
            {i < TIMELINE_STEPS.length - 1 && (
              <View
                style={[
                  styles.timelineLine,
                  {backgroundColor: step.done ? step.color + '40' : Colors.borderMedium},
                ]}
              />
            )}
            <View
      ...
      <View style={styles.kpiGrid}>
        {KPI_DATA.map((kpi, i) => (
          <Animated.View
            ...
            <View style={[styles.kpiDelta, {backgroundColor: kpi.deltaColor + '12'}]}>
