...
const monitoredScreens: Record<ShowcaseRouteKey, ComponentType> = {
  ...
  Widgets: withScreenQuality('Widgets', WidgetsScreen),
  WindowControls: withScreenQuality('WindowControls', WindowControlsScreen),
...
export const showcaseRoutes: ShowcaseRouteDefinition[] = showcaseRegistry.map(
  screen => ({
    routeKey: screen.routeKey,
    component: monitoredScreens[screen.routeKey],
    options: createShowcaseScreenOptions(
      ('headerTitle' in screen ? screen.headerTitle : undefined) ??
        screen.title,
    ),
  }),
);
