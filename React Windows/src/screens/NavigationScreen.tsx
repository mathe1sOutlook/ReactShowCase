...
const TABS = [
  {
    ...
    summary: 'Top tabs and preview flow',
  },
  {
    key: 'orders',
    title: 'Orders',
    accent: Colors.secondary,
    summary: 'Checkout and delivery state',
  },
  {
    key: 'profile',
    title: 'Profile',
...
> = {
  studio: [
    {
      title: 'Overview',
      body: 'Top tabs host a swipeable pager while each tab keeps its own nested route depth.',
      metric: 'Tabs + swipe',
    },
    {
      title: 'Showcase',
      body: 'A pushed route simulates a detail screen inside the selected tab without leaving the pager.',
      metric: 'Nested push',
    },
    {
      title: 'Prototype',
      body: 'A third route demonstrates deeper navigation before returning to the root screen.',
      metric: 'Depth 3',
    },
  ],
  orders: [
    {
      title: 'Cart',
      body: 'The orders tab preserves its own state when the user switches to another top tab.',
      metric: 'State kept',
    },
    {
      title: 'Tracking',
      body: 'This mirrors a common desktop shell: tabs on top, stack flow inside the selected area.',
      metric: 'Per-tab stack',
    },
    {
      title: 'Receipt',
      body: 'The final route can be popped independently while the other tabs keep their own depth.',
  ...
  profile: [
    {
      title: 'Summary',
      body: 'Profile routes are often reached from drawers and sheets while the surrounding shell stays mounted.',
      metric: 'Drawer entry',
    },
    {
      title: 'Security',
      body: 'Bottom sheet navigation can jump directly into deeper account routes.',
      metric: 'Sheet route',
    },
    {
      title: 'Preferences',
      body: 'Pagination dots below the pager reinforce progress while tabs remain available above.',
...
export default function NavigationScreen() {
  ...
  const safePagerWidth = pagerWidth || width - 40 - Spacing.xl;
              ...
              {TABS.map(tab => {
                ...
                return (
                  <View
                    ...
                    style={[
                      styles.page,
                      {width: safePagerWidth, borderColor: tab.accent + '33'},
                    ]}>
                    ...
                    <View style={styles.actionRow}>
                      ...
                      <Pressable
                        style={[
                          styles.actionButton,
                          {backgroundColor: tab.accent + '14', borderColor: tab.accent},
        ...
        <View style={styles.drawerList}>
          {drawerActions.map((item, index) => (
            <Animated.View
              ...
              <Pressable
                style={[styles.drawerItem, {borderColor: item.accent + '33'}]}
      ...
      <Animated.View
        ...
        <View style={styles.sheetList}>
          {sheetActions.map(item => (
            <Pressable
              key={item.label}
              style={[styles.sheetItem, {borderColor: item.accent + '33'}]}
              onPress={item.onPress}>
