...
export default function PlatformScreen() {
  ...
  const [clipboardText, setClipboardText] = useState('cfdwindows://files');
  const [clipboardCache, setClipboardCache] = useState('cfdwindows://files');
  ...
  const localizedCopy = {
    'en-US': {
      ...
      body: 'Clipboard sync, route previews, notifications and accessibility are active.',
    },
    'pt-BR': {
      title: 'Recursos do sistema online',
      body: 'Clipboard, previews de rota, notificacoes e acessibilidade estao ativas.',
    },
    'es-ES': {
  ...
  const sharePayload = async (kind: 'text' | 'url' | 'image') => {
    const payload =
      kind === 'text'
        ...
        : kind === 'url'
          ? {message: 'Open the device lab route', url: Platform.OS === 'android' ? 'cfdandroid://network' : 'cfdwindows://network'}
